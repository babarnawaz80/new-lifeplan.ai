...
// speech with ElevenLabs. Returns streaming MP3 bytes, or 204 when the voice
// service isn't configured so the browser voice can take over.
//
// GET is the low-latency path: the browser can start playing the audio while
// ElevenLabs is still generating it. POST is kept for callers that prefer it.
...
async function synthesize(text: string): Promise<Response> {
  ...
  try {
    const res = await fetch(
      `https://api.elevenlabs.io/v1/text-to-speech/${VOICE_ID}/stream?output_format=mp3_22050_32&optimize_streaming_latency=3`,
      {
        method: "POST",
        headers: { "xi-api-key": apiKey, "Content-Type": "application/json" },
        body: JSON.stringify({
          text: clean,
          // Fastest ElevenLabs model — noticeably lower time-to-first-audio.
