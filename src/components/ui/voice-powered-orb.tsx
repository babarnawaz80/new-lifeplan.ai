// A WebGL orb that reacts to the microphone level. Rendered client-side only.
...
interface VoicePoweredOrbProps {
  ...
  /** Extra activity (0..1) that animates the orb while it is speaking. */
  activity?: number;
...
export const VoicePoweredOrb: FC<VoicePoweredOrbProps> = ({
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      let micReady = false;
      if (voiceRef.current) micReady = await initMic();
      ...
      const update = (t: number) => {
        ...
        detectedCb.current?.(mic > 0.1);
        rot += dt * (0.3 + energy * maxRotationSpeed * 2.0);
        program.uniforms.rot.value = rot;
        program.uniforms.hover.value = Math.min(energy * 2.0, 1.0);
        program.uniforms.hoverIntensity.value = Math.min(
          energy * maxHoverIntensity * 0.8,
          maxHoverIntensity,
        );
